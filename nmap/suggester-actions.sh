...
port_80_443(){
	...
	# ------ DIR Busting ------
	echo -e "\n\t\t${DARK_GREEN}>>>>>> DIR Busting${NC}"
	
	# ------ dirsearch --------
	echo -e "\t\t\t-----> dirsearch <-----"
	echo -e "\t\t  Start with dirsearch in background" | $tee_command
	echo -e "\t\t   ${LIGHT_MAGENTA}dirsearch -u http://$IP_ADDRESS${1:+:}$1 -o `pwd`/$OUTPUT_DIR/dirsearch${1:+:}$1.out${NC}" | $tee_command
	# ------ Gobuster ---------
	echo -e "\t\t\t-----> gobuster <-----"
	if ! [[ -n "$mediumDirectory" ]]; then
		mediumDirectory="<PATH_TO_DIR/subdomains-top1million-5000.txt>"
	fi
	echo -e "\t\t  As secondary command use gobuster:" | $tee_command
...
PORT_MESSAGES[22]="\t${RED}Port 22${NC}: SSH (Secure Shell)
		${LIGHT_MAGENTA}ssh ${RED}someName${LIGHT_MAGENTA}@$IP_ADDRESS${NC}
		Visit ${BLUE}https://book.hacktricks.xyz/network-services-pentesting/pentesting-ssh${NC}
	----------------------------------------------------------"
